...
define('KEALOA_DB_VERSION', '2.3.7');
...
require_once KEALOA_PLUGIN_DIR . 'includes/class-kealoa-badges.php';
...
function kealoa_init(): void {
    ...
    // Flush caches when plugin version changes (HTML output may have changed)
    $installed_plugin_version = get_option('kealoa_plugin_version', '0');
    if (version_compare($installed_plugin_version, KEALOA_VERSION, '<')) {
        update_option('kealoa_plugin_version', KEALOA_VERSION);
        Kealoa_Shortcodes::flush_all_caches();
    }
...
function kealoa_render_bug_report_button(): void {
    ...
    <button class="kealoa-bug-report-btn" type="button" title="<?php esc_attr_e('Report a Bug', 'kealoa-reference'); ?>">
        <img src="<?php echo esc_url(KEALOA_PLUGIN_URL . 'assets/images/bug-report.svg'); ?>" alt="<?php esc_attr_e('Report a Bug', 'kealoa-reference'); ?>" width="24" height="24">
    </button>
